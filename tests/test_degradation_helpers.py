...
from battery_sim.degradation import _arrhenius_factor, _c_rate_factor, _soc_calendar_factor
...
def test_arrhenius_factor_activation_energy_zero_disables_temperature_stress(temp_degC: float):
    assert _arrhenius_factor(temp_degC, 25.0, 0.0) == pytest.approx(1.0)


def test_arrhenius_factor_increases_with_higher_temperature():
    assert _arrhenius_factor(35.0, 25.0, 4000.0) > _arrhenius_factor(25.0, 25.0, 4000.0)


def test_arrhenius_factor_decreases_with_lower_temperature():
    assert _arrhenius_factor(15.0, 25.0, 4000.0) < _arrhenius_factor(25.0, 25.0, 4000.0)


def test_arrhenius_factor_is_monotonic_in_temperature():
    factors = [_arrhenius_factor(temp, 25.0, 4000.0) for temp in [0.0, 10.0, 20.0, 30.0, 40.0]]

    assert factors == sorted(factors)
    assert len(set(factors)) == len(factors)


@pytest.mark.parametrize("temp_degC", [math.nan, math.inf, -math.inf, -273.15, -300.0])
...
def test_soc_calendar_factor_interpolates_lower_region(soc: float, expected: float):
    assert _soc_calendar_factor(soc, 0.2, 0.8, 1.5, 2.0) == pytest.approx(expected)
...
def test_soc_calendar_factor_interpolates_upper_region(soc: float, expected: float):
    assert _soc_calendar_factor(soc, 0.2, 0.8, 1.5, 2.0) == pytest.approx(expected)
