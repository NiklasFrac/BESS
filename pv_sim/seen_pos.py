...
def compute_apparent_sun_position(
    meteo_path: Path,
    true_solar_path: Path,
    out_path: Path,
    freq: str,
    missing_value: float,
) -> None:
    ...
    for col in ["TT_10", "PP_10"]:
        ...
        meteo[col] = meteo[col].replace(missing_value, pd.NA)
    ...
    log.info(
        "Gespeichert: %s | Zeilen: %d | mit Meteo: %d | ohne: %d",
        out_path,
        len(df),
        int(df["met_data_complete"].sum()),
        int((~df["met_data_complete"]).sum()),
    )