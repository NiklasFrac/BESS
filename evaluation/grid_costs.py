...
def _add_grid_cost_columns(
    ...
    df["demand_increment_kw"] = (
        df["grid_peak_so_far_kw"].diff().fillna(df["grid_peak_so_far_kw"]).clip(lower=0)
    )
    df["demand_increment_cost_eur"] = (
        df["demand_increment_kw"] * demand_charge_eur_per_kw_year
    )
    return df
...
def write_load_grid_costs(
    ...
    *,
    ...
    out = _add_grid_cost_columns(
        out, energy_price_eur_per_kwh, demand_charge_eur_per_kw_year, dt_h
    )
...
def write_system_grid_costs(
    ...
    *,
    ...
    pv = (
        pd.concat(
            pv.assign(
                timestamp_utc=pv["timestamp_utc"]
                - pd.Timedelta(minutes=pv_min)
                + pd.Timedelta(minutes=i * base_min)
            )
            for i in range(pv_min // base_min)
        )
        .set_index("timestamp_utc")
        .resample(f"{target_min}min")
        .mean()
        .dropna()
        .reset_index()
    )
    ...
    out = battery.merge(pv, on="timestamp_utc").rename(
        columns={"actual_kw": "actual_battery_action_kw"}
    )
    for df in (load, out):
        df["merge_key"] = df["timestamp_utc"].dt.strftime("%m-%d %H:%M")
        df["_n"] = df.groupby("merge_key").cumcount()
    out = out.merge(load[["merge_key", "_n", "load_kw"]], on=["merge_key", "_n"]).drop(
        columns=["merge_key", "_n"]
    )
    out["grid_import_kw"] = (
        out["load_kw"] - out["pv_kw"] + out["actual_battery_action_kw"]
    ).clip(lower=0)
    out = _add_grid_cost_columns(
        out, energy_price_eur_per_kwh, demand_charge_eur_per_kw_year, dt_h
    )
