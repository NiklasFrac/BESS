...
def validate_degradation_spec(spec: dict) -> None:
    ...
    if not (
        ...
        raise ValueError(
            "Require 0 < calendar_low_soc_reference < calendar_high_soc_reference < 1."
        )
...
def _soc_calendar_factor(
    ...
    if soc > high_soc_reference:
        return 1.0 + (high_soc_factor - 1.0) * (soc - high_soc_reference) / (
            1.0 - high_soc_reference
        )
...
def update_degradation_for_period(
    ...
    for temp in battery_temp_degC_series:
        if not math.isfinite(temp) or temp <= -273.15:
            raise ValueError(
                "battery_temp_degC_series values must be finite and above absolute zero."
            )
    ...
    if len(soc_fraction_series) != len(battery_temp_degC_series):
        raise ValueError(
            "soc_fraction_series and battery_temp_degC_series must have same length."
        )
    if len(power_kW_series) != len(soc_fraction_series):
        raise ValueError(
            "power_kW_series and soc_fraction_series must have same length."
        )
    ...
    base_calendar_fade = spec["calendar_fade_at_1yr"] * math.sqrt(new_years) - spec[
        "calendar_fade_at_1yr"
    ] * math.sqrt(old_years)
    ...
    if len(soc_fraction_series) >= 2:
        for rng, _mean, count, i_start, i_end in rainflow.extract_cycles(
            soc_fraction_series
        ):
            dod = rng
            if dod <= 0:
    ...
    new_state["capacity_factor"] = max(
        0.0, (1.0 - new_state["cycle_fade"]) * (1.0 - new_state["calendar_fade"])
    )
